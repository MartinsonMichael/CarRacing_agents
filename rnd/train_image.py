...
def main():
    ...
    input_size = phi(env.reset()).shape
    ...
    while True:
        ...
        for _ in range(num_step):
            ...
            actions, value_ext, value_int, policy = agent.get_action(np.array([phi(x) for x in states]))
            ...
            next_states = np.stack([phi(x) for x in next_states])
        ...
        _, value_ext, value_int, _ = agent.get_action(np.array([phi(x) for x in states]))
        total_ext_values.append(value_ext)
        total_int_values.append(value_int)
        # --------------------------------------------------
        ...
        #                   total_policy)
        agent.train_model(np.array([phi(x) for x in total_state]), ext_target, int_target, total_action,
                          total_adv, ((total_next_obs - obs_rms.mean) / np.sqrt(obs_rms.var)).clip(-5, 5),
                          total_policy)
