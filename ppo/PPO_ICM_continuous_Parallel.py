...
class PPO_ICM_parallel:
    def __init__(self, config: Config):
        ...
        self.memory = Torch_Parallel_Arbitrary_Replay_Buffer(
            buffer_size=10 ** 4,
            batch_size=10 ** 4,
            batch_add=True,
            done_key_word='done',
    ...
    def train(self):
        for index in range(self.hyperparameters['num_episodes_to_run']):
            ...
            self.memory.remove_all_not_done_session()
            self.update()
    ...
    def run_one_episode(self, eval=False):
        ...
        for _ in range(self.hyperparameters['max_episode_len'] + 1):
            # Running policy_old:
            action, log_prob, _ = self.ac.sample_action(state, to_numpy=True, remove_batch=False)
            next_state, reward, done, info = self.env.step(action)
            self.global_step_number += 1
            self.update_current_game_stats(reward[0], done[0], info[0])
            ...
            if done \
                ...
                if record_anim:
                    Process(
                        ...
                        args=(
                            images,
                            f'animation/{self.name}/_R:_{total_reward}_Time:_{episode_len}_{time.time()}.mp4',
                            self.stat_logger
                        ),
                    ).start()
